...
import Artifact__USDB from '@eth-optimism/contracts-bedrock/forge-artifacts/USDB.sol/USDB.json'
...
import Artifact__L1BlastBridge from '@eth-optimism/contracts-bedrock/forge-artifacts/L1BlastBridge.sol/L1BlastBridge.json'
...
task('deposit-erc20-blast', 'Deposits USDC onto L2 through the Blast Bridge.')
  .addParam(
    'l2ProviderUrl',
    'L2 provider URL.',
    'http://localhost:9545',
  ...
  .setAction(async (args, hre) => {
    ...
    if (args.l1ContractsJsonPath) {
      ...
      contractAddrs = {
        l1: {
          ...
          L1BlastBridge: json.L1BlastBridgeProxy,
    ...
    console.log(`L1BlastBridge: ${contractAddrs.l1.L1BlastBridge}`)
    const L1BlastBridge = new hre.ethers.Contract(
      contractAddrs.l1.L1BlastBridge,
      Artifact__L1BlastBridge.abi,
      signer
    )
    ...
    const USDB = new hre.ethers.Contract(
      predeploys.USDB,
      Artifact__USDB.abi
    )
    ...
    console.log('Depositing USDC to L2')
    ...
    const l2Balance = await USDB.balanceOf(address)
    ...
    for (const log of finalizeReceipt.logs) {
      switch (log.address) {
        ...
        case L1BlastBridge.address: {
          const parsed = L1BlastBridge.interface.parseLog(log)
          console.log(
