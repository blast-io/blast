...
func (c *ecrecoverOracle) Run(caller common.Address, input []byte, db vm.StateDB, readOnly bool) ([]byte, error) {
...
func (b *bn256PairingOracle) Run(caller common.Address, input []byte, db vm.StateDB, readOnly bool) ([]byte, error) {
...
func (b *kzgPointEvaluationOracle) Run(caller common.Address, input []byte, db vm.StateDB, readOnly bool) ([]byte, error) {
	// Modification note: the L1 precompile behavior may change, but not in incompatible ways.
	// We want to enforce the subset that represents the EVM behavior activated in L2.
	// Below is a copy of the Cancun behavior. L1 might expand on that at a later point.
