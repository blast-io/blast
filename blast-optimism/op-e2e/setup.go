...
func (sys *System) L1BeaconHTTPClient() *sources.BeaconHTTPClient {
	logger := testlog.Logger(sys.t, log.LvlInfo).New("component", "beaconClient")
...
func (cfg SystemConfig) Start(t *testing.T, _opts ...SystemConfigOption) (*System, error) {
	...
	l1Genesis, err := genesis.BuildL1DeveloperGenesis(cfg.DeployConfig, config.L1Allocs, config.L1Deployments, false)
	if err != nil {
		return nil, err
	}
	...
	snapLog := log.New(log.DiscardHandler())
	...
	if e2eutils.UseFPAC() {
		return nil, errors.New("blast doesnt use FPAC atm")
...
func configureL1(rollupNodeCfg *rollupNode.Config, l1Node EthInstance) {
	...
	rollupNodeCfg.L1 = &rollupNode.L1EndpointConfig{
		...
		L1TrustRPC:       true, // NOTE(Blast) because change done to AccountResult and otherwise Verify blows up
