...
def devnet_deploy(paths):
    if os.path.exists(paths.genesis_l1_path):
        ...
        state = read_json(paths.allocs_path)
        fixed_state = zero_pad_hex_recursive(state)
        write_json(paths.allocs_path, fixed_state)
...
def debug_dumpBlock(url):
    ...
    conn = http.client.HTTPConnection(url)
    headers = {'Content-type': 'application/json'}
    body = '{"id":3, "jsonrpc":"2.0", "method": "anvil_dumpState"}'
    conn.request('POST', '/', body, headers)
    response = conn.getresponse()
    data = response.read().decode()
    conn.close()
...
def zero_pad_hex_recursive(data):
    if isinstance(data, list):
        return [zero_pad_hex_recursive(item) for item in data]
    elif isinstance(data, dict):
        res = {}
        for key, value in data.items():
            if key == 'storage':
                res[key] = zero_pad_storage(value)
            elif key == 'balance':
                res[key] = str(int(value, 16))
            else:
                res[key] = zero_pad_hex_recursive(value)
        return res
    else:
        return data

def zero_pad_storage(data):
    return {zero_pad(key): zero_pad(value) for key, value in data.items()}

def zero_pad(data):
    return '0x' + data.lstrip('0x').rjust(64, '0')
