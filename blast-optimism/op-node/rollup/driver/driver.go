...
func NewDriver(driverCfg *Config, cfg *rollup.Config, l1ChainConfig *params.ChainConfig, l2 L2Chain, l1 L1Chain, l1Blobs derive.L1BlobsFetcher, altSync AltSync, network Network, log log.Logger, snapshotLog log.Logger, metrics Metrics, sequencerStateListener SequencerStateListener, syncCfg *sync.Config) *Driver {
	...
	derivationPipeline := derive.NewDerivationPipeline(log, cfg, verifConfDepth, l1Blobs, l2, engine, metrics, syncCfg, l1ChainConfig)
	attrBuilder := derive.NewFetchingAttributesBuilder(cfg, l1ChainConfig, l1, l2)
