...
func (gtm *GasTracker) AllocateDevGas(gasPrice *big.Int, refund uint64, state StateDB, timestamp uint64) {
	...
	if metrics.Enabled {
		defer func(start time.Time, amount int) {
			gtm.AllocateDevGasUpdates += time.Since(start)
			gtm.AllocateCount += amount
		}(time.Now(), len(gtm.allocations))
	}
	...
	blastGasUnits := new(big.Int).Sub(remainingGas, accumulatedGas)
	blastGas := new(big.Int).Mul(blastGasUnits, gasPrice)
	state.AddBalance(params.OptimismBaseFeeRecipient, blastGas)

	// pay out non-void gas to blast predeploy
	claimableGasToAdd := new(big.Int).Mul(accumulatedGas, gasPrice)
	state.AddBalance(params.BlastGasAddress, claimableGasToAdd)
}
...
func readGasParameters(state StateDB, contractAddress common.Address) *GasParameters {
	...
	gasStorageSlotBytes := state.GetState(params.BlastGasAddress, slot).Bytes()
	gasParameters, err := unpack(gasStorageSlotBytes)
	if err != nil {
		// TODO(blast): remove this panic, should never happen
		panic(err)
	}
...
func updateGasParameters(state StateDB, contractAddress common.Address, gasParameters *GasParameters) {
	...
	state.SetState(params.BlastGasAddress, slot, common.BytesToHash(byteArray))
}
