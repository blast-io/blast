...
var (
	...
	devAllocateUpdateTimer = metrics.NewRegisteredTimer("chain/gastracker/updates", nil)
...
type BlockChain struct {
	...
	allTxReceiptsCache *lru.Cache[common.Hash, *types.Receipt]
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc := &BlockChain{
		...
		allTxReceiptsCache: lru.NewCache[common.Hash, *types.Receipt](1024),
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		for _, r := range receipts {
			bc.allTxReceiptsCache.Add(r.TxHash, r)
		}
		...
		devAllocateUpdateTimer.Update(statedb.DevAllocateUpdates)     // GasTracker updates are complete(in processing)
		...
		stats.report(chain, it.index, snapDiffItems, snapBufItems, trieDiffNodes, trieBufNodes, setHead, statedb.DevAllocateUpdates, statedb.TrieDBCommits, statedb.StorageCommits, statedb.AccountCommits, statedb.DevAllocateCount)
...
func (bc *BlockChain) FastReceiptCache(hsh common.Hash) *types.Receipt {
	rcpt, _ := bc.allTxReceiptsCache.Get(hsh)
	return rcpt
}
